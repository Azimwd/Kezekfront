import {
    Banknote
} from 'lucide-react';

import type {
    BookingSettings
} from '../../../../api/settings';


interface Props {
    settings: BookingSettings;

    updateField: <
        K extends keyof BookingSettings
    >(
        field: K,
        value: BookingSettings[K]
    ) => void;
}


export default function PrepaymentSettings({
    settings,
    updateField
}: Props) {
    return (
        <section
            className="
                w-full
                min-w-0
                rounded-2xl
                border
                border-[#cbc9df]
                bg-white
                p-4
                sm:p-5
                md:p-6
            "
        >
            <div
                className="
                    flex
                    min-w-0
                    items-center
                    gap-3
                "
            >
                <div
                    className="
                        flex
                        h-9
                        w-9
                        shrink-0
                        items-center
                        justify-center
                        rounded-full
                        bg-[#f1efff]
                        sm:h-10
                        sm:w-10
                    "
                >
                    <Banknote
                        className="
                            h-4
                            w-4
                            text-[#4031d0]
                            sm:h-5
                            sm:w-5
                        "
                    />
                </div>

                <h2
                    className="
                        min-w-0
                        text-lg
                        font-semibold
                        leading-6
                        text-slate-900
                        sm:text-xl
                    "
                >
                    Предоплата
                </h2>
            </div>

            <div
                className="
                    mt-5
                    flex
                    min-w-0
                    flex-col
                    gap-4
                    sm:mt-7
                    sm:flex-row
                    sm:items-center
                    sm:justify-between
                    sm:gap-6
                "
            >
                <div
                    className="
                        min-w-0
                        flex-1
                    "
                >
                    <div
                        className="
                            text-[13px]
                            font-semibold
                            leading-5
                            text-slate-700
                            sm:text-sm
                        "
                    >
                        Требовать предоплату
                    </div>

                    <p
                        className="
                            mt-1.5
                            max-w-[530px]
                            text-[12px]
                            leading-5
                            text-slate-500
                            sm:text-[13px]
                        "
                    >
                        Клиент должен внести предоплату,
                        прежде чем запись будет подтверждена.
                    </p>
                </div>

                <div
                    className="
                        flex
                        w-full
                        justify-end
                        sm:w-auto
                    "
                >
                    <Toggle
                        checked={
                            settings.prepayment_enabled
                        }
                        onChange={() =>
                            updateField(
                                'prepayment_enabled',
                                !settings.prepayment_enabled
                            )
                        }
                    />
                </div>
            </div>

            {settings.prepayment_enabled && (
                <div
                    className="
                        mt-6
                        grid
                        grid-cols-1
                        gap-5
                        border-t
                        border-[#ecebf4]
                        pt-6
                        md:grid-cols-[180px_minmax(0,1fr)]
                    "
                >
                    <div>
                        <label
                            className="
                                block
                                text-[13px]
                                font-semibold
                                text-slate-700
                            "
                        >
                            Размер предоплаты
                        </label>

                        <p
                            className="
                                mt-1
                                text-[12px]
                                leading-5
                                text-slate-500
                            "
                        >
                            Процент от полной стоимости записи.
                        </p>
                    </div>

                    <div
                        className="
                            relative
                            max-w-[180px]
                        "
                    >
                        <input
                            type="number"
                            min={1}
                            max={100}
                            value={
                                settings.prepayment_percent
                            }
                            onChange={
                                event =>
                                    updateField(
                                        'prepayment_percent',
                                        Number(
                                            event.target.value
                                        )
                                    )
                            }
                            className="
                                h-11
                                w-full
                                rounded-xl
                                border
                                border-[#cbc9df]
                                bg-white
                                px-4
                                pr-11
                                text-sm
                                font-medium
                                text-slate-900
                                outline-none
                                transition
                                focus:border-[#4a38e8]
                                focus:ring-2
                                focus:ring-[#4a38e8]/10
                            "
                        />

                        <span
                            className="
                                pointer-events-none
                                absolute
                                right-4
                                top-1/2
                                -translate-y-1/2
                                text-sm
                                font-medium
                                text-slate-500
                            "
                        >
                            %
                        </span>
                    </div>

                    <div>
                        <label
                            className="
                                block
                                text-[13px]
                                font-semibold
                                text-slate-700
                            "
                        >
                            Ссылка Kaspi
                        </label>

                        <p
                            className="
                                mt-1
                                text-[12px]
                                leading-5
                                text-slate-500
                            "
                        >
                            Ссылка будет отправлена клиенту
                            для внесения предоплаты.
                        </p>
                    </div>

                    <div>
                        <input
                            type="url"
                            value={
                                settings.kaspi_payment_url
                            }
                            onChange={
                                event =>
                                    updateField(
                                        'kaspi_payment_url',
                                        event.target.value
                                    )
                            }
                            placeholder="https://..."
                            className="
                                h-11
                                w-full
                                rounded-xl
                                border
                                border-[#cbc9df]
                                bg-white
                                px-4
                                text-sm
                                text-slate-900
                                outline-none
                                transition
                                placeholder:text-slate-400
                                focus:border-[#4a38e8]
                                focus:ring-2
                                focus:ring-[#4a38e8]/10
                            "
                        />
                    </div>
                </div>
            )}
        </section>
    );
}


function Toggle({
    checked,
    onChange
}: {
    checked: boolean;
    onChange: () => void;
}) {
    return (
        <button
            type="button"
            role="switch"
            aria-checked={checked}
            onClick={onChange}
            className={`
                relative
                h-7
                w-12
                shrink-0
                cursor-pointer
                rounded-full
                transition-colors
                duration-200
                ${
                    checked
                        ? 'bg-[#4a38e8]'
                        : 'bg-slate-300'
                }
            `}
        >
            <span
                className={`
                    absolute
                    left-1
                    top-1
                    h-5
                    w-5
                    rounded-full
                    bg-white
                    shadow-sm
                    transition-transform
                    duration-200
                    ${
                        checked
                            ? 'translate-x-5'
                            : 'translate-x-0'
                    }
                `}
            />
        </button>
    );
}